...
#mu_AT=mutrates$Probability[mutrates$Nucleotide.substitution=="AU"]/5
mu_AC=9*10^-7
mu_AT=(7*10^-7)
#mu_AT=(7*10^-7)/5
...
Ne = 300000 #currently Ne cannot be changed in the sims, has to be 10000 # 07/11/17 I changed it to 100000
...
#Get sample Freqs
#n=500000
#Freqs103AAC_sim<-rbinom(200,n,PopFreqs103AAC_sim)/n
#Plot
br<-seq(-0.00001+10^-7,0.1,by=0.00001)

par(mfrow=c(2,2))
#png("Freqs103AAC.png")
hist(Freqs103AAC,xlim=c(-0.00001+10^-7,0.0002),breaks=br, 
...
hist(PopFreqs103AAC_sim,xlim=c(-0.00001+10^-7,0.0002),breaks=br, 
     ylab="", xlab = "", main="Simul.Single-Site Frequency Spectrum RT103AAC",
     xaxt="n", col="blue"
)
axis(1, at=c(round(br[2],5)-0.000005,round(br[3:16],5)),labels=round(br[2:16],5), col.axis="black", las=2)
...
mtext("Observed frequency", side=1, line=4, cex.lab=1,las=1, col="black")
mtext("Number of patients", side=2, line=2.5, cex.lab=1,las=3, col="black")
abline(v=mean(PopFreqs103AAC_sim),col=2)

hist(Freqs103AAT,xlim=c(-0.00001+10^-7,0.0002),breaks=br, 
...
hist(PopFreqs103AAT_sim,xlim=c(-0.00001+10^-7,0.0002),breaks=br, 
     ylab="", xlab = "", main="Simul.Single-Site Frequency Spectrum RT103AAT",
     xaxt="n", col="blue"
)
axis(1, at=c(round(br[2],5)-0.000005,round(br[3:16],5)),labels=round(br[2:16],5), col.axis="black", las=2)
...
mtext("Observed frequency", side=1, line=4, cex.lab=1,las=1, col="black")
mtext("Number of patients", side=2, line=2.5, cex.lab=1,las=3, col="black")
abline(v=mean(PopFreqs103AAT_sim),col=2)
